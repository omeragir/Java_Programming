...
public abstract class Car {  //meant to be inherited , can not be instantiated
    ...
    public Car(String model, String color, int year, double price) {
        this.make = getClass().getSimpleName();//    to set the class name to make of the car
        this.model = model;
        setColor(color);
        if (year < 1886) {
    ...
    public void setPrice(double price) {
        if (price <= 0) {
            System.err.println("Price can not be less than negative or zero " + price);
            System.exit(1);
        }
        this.price = price;
    }

    protected final void stop(){  //to make sure that non-subclass outside package can not access it
